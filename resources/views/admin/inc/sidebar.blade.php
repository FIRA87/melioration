@php
    $id = Auth::user()->id;
    $userid = App\Models\User::find($id);
    $status = $userid->status;
@endphp

<div class="left-side-menu">
<div class="h-100" data-simplebar>
<!--- Sidemenu -->
<div id="sidebar-menu">
<ul id="side-menu">
    <li class="menu-title">Навигация</li>
    <li>
        <a href="{{ route('admin.dashboard') }}">
            <i class="mdi mdi-view-dashboard-outline"></i>
            <span> Панель инструментов </span>
        </a>
    </li>
@if(Auth::user()->can('media.index'))
  
<li>
    <a href="#media" data-bs-toggle="collapse">
        <i class="mdi mdi-folder-multiple-image"></i>
        <span> Медиабиблиотека </span>
        <span class="menu-arrow"></span>
    </a>
    <div class="collapse" id="media">
        <ul class="nav-second-level">
            <li style="margin-left: -20px"> <a href="{{ route('media.index') }}"><i
                        class="mdi mdi-animation"></i> Все </a></li>
        </ul>
    </div>
</li>

@endif


@if(Auth::user()->can('menu.menu'))
        <li>
            <a href="#page" data-bs-toggle="collapse">
                <i class="mdi mdi-menu"></i>
                <span> Меню </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="page">
                <ul class="nav-second-level">
@if(Auth::user()->can('menu.list'))
    <li> <a href="{{ route('all.pages') }}"> <i class="mdi mdi-animation"></i> Все</a></li>
@endif 

@if(Auth::user()->can('menu.add'))
    <li> <a href="{{ route('add.pages') }}"> <i class="mdi mdi-database-plus"></i>Добавить</a> </li>
@endif 

                </ul>
            </div>
        </li>
@endif 


@if(Auth::user()->can('submenu.menu'))
        <li>
            <a href="#page-munu" data-bs-toggle="collapse">
                <i class="mdi mdi-menu-open"></i>
                <span>Подменю </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="page-munu">
                <ul class="nav-second-level">

@if(Auth::user()->can('submenu.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.submenu') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('submenu.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.submenu') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif 


                </ul>
            </div>
        </li>
@endif 


@if(Auth::user()->can('leader.menu'))
        <li>
            <a href="#sidebarLeader" data-bs-toggle="collapse">
                <i class="fa-solid fa-folder-tree"></i>
                <span> Руководство </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarLeader">
                <ul class="nav-second-level">

@if(Auth::user()->can('leader.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('leader.index') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('leader.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('leader.create') }}"><i class="mdi mdi-database-plus"></i> Добавить</a>
    </li>
@endif 

                </ul>
            </div>
        </li>
@endif 

@if(Auth::user()->can('category.menu'))
        <li>
            <a href="#sidebarEcommerce" data-bs-toggle="collapse">
                <i class="mdi mdi-tag-multiple"></i>
                <span> Категория </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarEcommerce">
                <ul class="nav-second-level">

@if(Auth::user()->can('category.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.category') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('category.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.category') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif 


                </ul>
            </div>
        </li>
@endif 



@if(Auth::user()->can('links.menu'))
        <li>
            <a href="#Link" data-bs-toggle="collapse">
                <i class="mdi mdi-link"></i>
                <span>Партнёры </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="Link">
                <ul class="nav-second-level">

@if(Auth::user()->can('links.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.links') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('links.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.links') }}"><i class="mdi mdi-database-plus"></i> Добавить   </a>
    </li>
@endif 

                </ul>
            </div>
        </li>
@endif 



        <li class="menu-title mt-2">Контент</li>
@if(Auth::user()->can('news.menu'))
        <li>
            <a href="#sidebarAuth" data-bs-toggle="collapse">
                <i class="mdi mdi-newspaper"></i>
                <span> Настройка новостей </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarAuth">
                <ul class="nav-second-level">
@if(Auth::user()->can('news.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.news') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('news.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.news') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif 
                </ul>
            </div>
        </li>
@endif 

@if(Auth::user()->can('video.menu'))
        <li>
            <a href="#sidebarCrm" data-bs-toggle="collapse">
                <i class="mdi mdi-video"></i>
                <span> Настройка видео </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarCrm">
                <ul class="nav-second-level">

@if(Auth::user()->can('video.list'))
    <li style="margin-left: -20px">
        <a href="{{ url('all/video') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('video.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.video') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif 

                </ul>
            </div>
        </li>
@endif 

@if(Auth::user()->can('gallery.menu'))
        <li>
            <a href="#sidebarExpages" data-bs-toggle="collapse">
                <i class="mdi mdi-image-multiple"></i>
                <span>Галерея </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarExpages">
                <ul class="nav-second-level">
@if(Auth::user()->can('gallery.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.gallery') }}"><i class="mdi mdi-animation"></i> Все
        </a>
    </li>
@endif 

@if(Auth::user()->can('gallery.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.gallery') }}"><i class="mdi mdi-database-plus"></i>
            Добавить </a>
    </li>
@endif 

                </ul>
            </div>
        </li>
@endif 

@if(Auth::user()->can('presidents.menu'))
        <li>
            <a href="#sidebarPresident" data-bs-toggle="collapse">
                <i class="mdi mdi-account-star"></i>
                <span>Президент </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarPresident">
                <ul class="nav-second-level">
@if(Auth::user()->can('presidents.list'))                       
    <li style="margin-left: -20px">
        <a href="{{ route('all.presidents') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif 

@if(Auth::user()->can('presidents.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.presidents') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif 
                </ul>
            </div>
        </li>
@endif  


@if(Auth::user()->can('projects.menu'))
        <li>
            <a href="#sidebarProjects" data-bs-toggle="collapse">
                <i class="mdi mdi-folder-star"></i>
                <span>Проекты </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarProjects">
                <ul class="nav-second-level">

@if(Auth::user()->can('projects.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.projects') }}"><i class="mdi mdi-animation"></i> Все </a> 
    </li>
 @endif  

@if(Auth::user()->can('projects.add'))                    
    <li style="margin-left: -20px">
        <a href="{{ route('add.projects') }}"><i class="mdi mdi-database-plus"></i>Добавить </a>
    </li>
 @endif  
                </ul>
            </div>
        </li>
 @endif  

@if(Auth::user()->can('tasks.menu'))
        <li>
            <a href="#sidebarTasks" data-bs-toggle="collapse">
                <i class="mdi mdi-format-list-checks"></i>
                <span>Задачи </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarTasks">
                <ul class="nav-second-level">

@if(Auth::user()->can('tasks.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.tasks') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif  

@if(Auth::user()->can('tasks.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.tasks') }}"><i class="mdi mdi-database-plus"></i> Добавить</a> 
    </li>
@endif
                </ul>
            </div>
        </li>
 @endif


@if(Auth::user()->can('services.menu'))
        <li>
            <a href="#sidebarServices" data-bs-toggle="collapse">
                <i class="mdi mdi-cog-outline"></i>
                <span>Услуги </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarServices">
                <ul class="nav-second-level">

@if(Auth::user()->can('services.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.services') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif

@if(Auth::user()->can('services.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.services') }}"><i class="mdi mdi-database-plus"></i> Добавить </a>
    </li>
@endif

@if(Auth::user()->can('services.menu'))
    <li style="margin-left: -20px"> 
        <a href="{{ route('all.service.requests') }}"><i class="mdi mdi-animation"></i> Заявки на услуги </a>
    </li>
@endif

                </ul>
            </div>
        </li>

 @endif

@if(Auth::user()->can('surveys.menu'))
        <li>
            <a href="#sidebarPool" data-bs-toggle="collapse">
                <i class="fa-solid fa-square-poll-vertical"></i>
                <span>Голосования </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarPool">
                <ul class="nav-second-level">
@if(Auth::user()->can('surveys.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('surveys.index') }}"><i class="mdi mdi-animation"></i> Все </a>
    </li>
@endif   

@if(Auth::user()->can('surveys.add'))           
    <li style="margin-left: -20px">
        <a href="{{ route('surveys.create') }}"><i class="mdi mdi-database-plus"></i>Добавить </a>
    </li>
@endif            

                </ul>
            </div>
        </li>
@endif


@if(Auth::user()->can('contacts.menu'))
        <li>
            <a href="#sidebarContact" data-bs-toggle="collapse">
                <i class="fa-solid fa-paper-plane"></i>
                <span>Форма обратной связи </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarContact">
                <ul class="nav-second-level">
                    @if(Auth::user()->can('contacts.list'))
                    <li style="margin-left: -20px">
                        <a href="{{ route('contacts.index') }}"><i class="mdi mdi-animation"></i> Все
                        </a>
                    </li>
                     @endif
                </ul>
            </div>
        </li>
    @endif

@if(Auth::user()->can('jobs.menu'))
        <li>
            <a href="#sidebarJob" data-bs-toggle="collapse">
                <i class="fa-solid fa-briefcase"></i>
                <span>Вакансия </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarJob">
                <ul class="nav-second-level">
                    @if(Auth::user()->can('jobs.menu'))
                    <li style="margin-left: -20px">
                        <a href="{{ route('admin.jobs.index') }}"><i class="mdi mdi-animation"></i> Все    </a>
                    </li>
                     @endif
                    @if(Auth::user()->can('jobs.menu'))
                    <li style="margin-left: -20px">
                        <a href="{{ route('backend.applications.index') }}"><i class="mdi mdi-animation"></i> Заявки на вакансии</a>
                    </li>
                    @endif

                </ul>
            </div>
        </li>

 @endif

@if(Auth::user()->can('documents.menu'))
        <li>
            <a href="#sidebarDocument" data-bs-toggle="collapse">
                <i class="fa-solid fa-folder"></i>
                <span>Документы </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarDocument">
                <ul class="nav-second-level">
                    @if(Auth::user()->can('documents.list'))
                    <li style="margin-left: -20px">
                        <a href="{{ route('documents.index') }}"><i class="mdi mdi-animation"></i> Все
                        </a>
                    </li>
                     @endif
                </ul>
            </div>
        </li>
 @endif
@if(Auth::user()->can('static_translations_index'))
        <li class="nav-item">
            <a class="nav-link {{ Request::is('admin/static-translations*') ? 'active' : '' }}" 
               href="{{ route('static-translations.index') }}">
                <i class="link-icon" data-feather="globe"></i>
                <span class="link-title">Статические переводы</span>
            </a>
        </li>

 @endif



        <li class="menu-title mt-2">Настройка</li>
@if(Auth::user()->can('setting.menu'))
        <li>
            <a href="#setting" data-bs-toggle="collapse">
                <i class="mdi mdi-wrench"></i>
                <span> Настройка сайта </span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="setting">
                <ul class="nav-second-level">

<li style="margin-left: -20px">
    <a href="{{ route('setting.index') }}"><i class="mdi mdi-cog"></i>
        Настройка</a>
</li>

                </ul>
            </div>
        </li>
  @endif


@if(Auth::user()->can('admin.menu'))
        <li>
            <a href="#sidebarEmail" data-bs-toggle="collapse">
                <i class="mdi mdi-account-multiple"></i>
                <span> Пользователи</span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarEmail">
                <ul class="nav-second-level">

@if(Auth::user()->can('admin.list'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.admin') }}"><i class="mdi mdi-animation"></i> Все
            пользователи</a>
    </li>
@endif

@if(Auth::user()->can('admin.add'))
    <li style="margin-left: -20px">
        <a href="{{ route('add.admin') }}"><i class="mdi mdi-database-plus"></i> Добавить
        </a>
    </li>
@endif
                </ul>
            </div>
        </li>

@endif

@if(Auth::user()->can('all_permission'))
        <li>
            <a href="#sidebarRole" data-bs-toggle="collapse">
                <i class="mdi mdi-shield-half-full"></i>
                <span> Роли и права</span>
                <span class="menu-arrow"></span>
            </a>
            <div class="collapse" id="sidebarRole">
                <ul class="nav-second-level">
@if(Auth::user()->can('all_permission'))
    <li style="margin-left: -20px">
        <a href="{{ route('all.permission') }}"><i class="mdi mdi-account"></i> Все разрешения</a>
    </li>
 @endif     
 
@if(Auth::user()->can('all_roles'))      
    <li style="margin-left: -20px">
        <a href="{{ route('all.roles') }}"><i class="mdi mdi-account-key"></i> Роли пользователей</a>
    </li>
@endif  

@if(Auth::user()->can('add_roles_permission'))   
    <li style="margin-left: -20px">
        <a href="{{ route('add.roles.permission') }}"><i class="mdi mdi-account-key"></i>Добавить разрешение для роли</a>
    </li>
@endif  

@if(Auth::user()->can('all_roles_permission'))   
    <li style="margin-left: -20px">
        <a href="{{ route('all.roles.permission') }}"><i class="mdi mdi-database-plus"></i> Все роли с разрешениями</a>
    </li>
@endif  

                </ul>
            </div>
        </li>
        @endif
   
</ul>

</div>
<!-- End Sidebar -->

<div class="clearfix"></div>

</div>
<!-- Sidebar -left -->

</div>

                ...
                <tr>
                    ...
                    <td>
                        @if(Auth::user()->can('jobs.edit'))
                            <a href="{{ route('admin.jobs.edit', $job) }}" class="btn btn-sm btn-primary"><i class="fa-solid fa-pen"></i></a>
                        @endif
                        @if(Auth::user()->can('jobs.delete'))
                            <form action="{{ route('admin.jobs.destroy', $job) }}" method="POST" style="display:inline-block" onsubmit="return confirm('Удалить?')">
                                @csrf @method('DELETE')

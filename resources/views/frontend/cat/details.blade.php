                ...
                <div class="row justify-content-center">
                    <div class="col-xl-6 col-lg-8 col-md-10">
                        <div class="breadcrumb__wrap__content">
                            <h2 class="title">{{ $categoryname->category_en }}</h2>
                            <nav aria-label="breadcrumb">
                                <ol class="breadcrumb">
                                    <li class="breadcrumb-item"><a href="index.html">Home</a></li>
                                    <li class="breadcrumb-item active"
                                        aria-current="page">{{ $categoryname->category_en }}</li>
                                </ol>
                            </nav>
                        </div>
                    </div>
                        ...
                        @foreach($newspost as $item)
                            <div class="standard__blog__post">
                                <div class="standard__blog__thumb">
                                    ...
                                    <a href="{{ route('news.details', $item->id) }}" class="blog__link"><i
                                ...
                                <div class="standard__blog__content">
                                    ...
                                    <h2 class="title"><a
                                            href="{{ route('news.details', $item->id) }}">{{ $item->title_en }}.</a>
                                    </h2>
                                    <p> {!! Str::limit($item->desc_en, '250') !!} </p>
                                    ...
                                    @foreach($allnews as $item )
                                        <li class="rc__post__item">
                                            <div class="rc__post__thumb">
                                                <a href="{{ route('news.details', $item->id) }}"><img
                                                        src="{{ asset($item->image) }}" alt="{{ $item->title_en }}"></a>
                                            </div>
                                ...
                                <ul class="sidebar__cat">
                                    @foreach($categories as $cat)
                                        <li class="sidebar__cat__item"><a
                                                href="{{ route('category.news', $cat->id) }}">{{ $cat->category_en }}</a>
                                        </li>
                                    @endforeach
                                </ul>
                            </div>
